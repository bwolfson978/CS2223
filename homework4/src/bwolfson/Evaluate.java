...
public class Evaluate {
	public static void main(String[] args) {
		...
		while (!StdIn.isEmpty()) {
			...
			else if (s.equals("sqrt")){ops.push(new SquareRoot(s)); }
			
			else if (s.equals("^")){ops.push(new Exponent(s));}
			
			else if (s.equals("*")){ops.push(new Multiplication(s));}
			
			else if (s.equals("/")){ops.push(new Division(s));}
			
			else if (s.equals("-")){ops.push(new Subtraction(s));}
			
			// these are constants to be pushed onto the vals stack.
			else if (s.equals ("e")) { vals.push(new ENode(s)); }
			
			else if(s.equals ("pi")) {vals.push(new PI(s));}
